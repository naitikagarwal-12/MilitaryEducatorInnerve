...
function HomePage({ service }) {
  ...
  const handleAction = (card) => {
    if (card.title?.toLowerCase().includes("skill")) {
      navigate("/defense");
    }

    if (card.title?.toLowerCase().includes("mock")) {
      navigate("/mock");
    }
  };
    ...
    >
      ...
      <section className="flex flex-col items-center mt-8 sm:mt-10 gap-6 px-4">
        <div className="flex items-center gap-6 sm:gap-10">
          <img src={data.logo} alt={data.name} className="w-20 sm:w-24" />
      ...
      <section className="mt-12 px-4 sm:px-10 pb-16">
        ...
        >
          {data.cards.map((card, i) => (
            <div
              key={i}
              className={`border-2 ${t.border} p-4 flex flex-col gap-3`}
            >
